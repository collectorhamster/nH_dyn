...
F4 = Figure(fontsize=ftsz,size=(1400,900))
subf1 = F4[1,1] = GridLayout()
subf2 = F4[1,2] = GridLayout()
subf3 = F4[2,1] = GridLayout()
subf4 = F4[2,2] = GridLayout()
...
fin,ini = argmin(abs.(log.(x) .-2.0)),argmin(abs.(log.(x) .-0.0))
xfit = exp.(collect(log(x[ini]):0.1:log(x[fin])))
yfit = 1.2*y[ini]*(xfit ./xfit[1]).^(1)
CairoMakie.lines!(ax21,xfit,yfit;linestyle=(:dash,:dense),linewidth=2,color=:purple,label=L"s=1/2")

fin,ini = argmin(abs.(log.(x) .-7.5)),argmin(abs.(log.(x) .-5.5))
xfit = exp.(collect(log(x[ini]):0.1:log(x[fin])))
yfit = 1.25*y[ini]*(xfit ./xfit[1]).^(2/3)
...
rho = lapl(γ1_mat[:,:],0.05,0.05,2) ./(2*pi)
...
ax31 = Axis(subf3[1,1],xlabel=latexstring("\\mathrm{Im}E"),ylabel="iDOS",xlabelsize=xylbsz,ylabelsize=xylbsz
    ...
    ,xticks=[-2,0,2],yticks=[0,0.3]
    ,ylabelpadding=-20
)
ax31.ytickformat = y -> ["0","0.3"]
CairoMakie.barplot!(ax31,Ei_arr,idos,width=0.07)
...
CairoMakie.vlines!(ax31,2.0,linestyle=:dash,linewidth=2,color=:green)
...
,xtickalign=1,ytickalign=1,xticks=[cid1-2*wd,cid1-wd,length(state1)],yticks=[0,1])
...
CairoMakie.xlims!(ax41,(cid1-2*wd-5,length(state1)));CairoMakie.ylims!(ax41,(0,1))
